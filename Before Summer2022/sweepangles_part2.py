...
def sweep(numsamples):
    ...
    filenameSummary =  "anglesweep" + str(angleSetA) + str(angleSetB[1])
    currtime = dt.datetime.now().time() # HH:MM:SS.microseconds
    currtime = clean(str(currtime))
    filenameSummary += " at " + currtime + ".csv"
    savefilepath = os.path.join(basePathSummary, filenameSummary)
    ...
    with open(savefilepath, "w+") as csvfile:
        ...
        for x in anglesB: # Rotate Bob to this angle.
            # Reset the baseFilename variable
            baseFilename = basePath
            Bob_HWP.motor.move_to(x, True)
            #print(initialize_the_motors.aptWrapper.angle(Bob_HWP))
            #time.sleep(5)
